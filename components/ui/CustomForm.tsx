  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align='end'>
        {setFormStatusData && source === 'deposit' && (
          <>
            <DropdownMenuItem
              disabled={isPending}
              onClick={() => {
                setFormStatusData({ type: 'expense', id: original.id })
                setDialogOpen(true)
              }}
            >
              <FaDollarSign /> <span className='mx-1'> Expense</span>
            </DropdownMenuItem>
            <DropdownMenuItem
              disabled={isPending}
              onClick={() => {
                setFormStatusData({ type: 'deposit', id: original.id })
                setDialogOpen(true)
              }}
            >
              <FaDollarSign /> <span className='mx-1'> Deposit</span>
            </DropdownMenuItem>
          </>
        )}
